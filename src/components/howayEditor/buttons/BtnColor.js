...
class BtnColor extends Component {
    ...
    isInFocusElements = () => {
        ...
        for (let i in focusElements) {
            for(let j=0;j<values.length;j++){
                if(values[j].toLowerCase()===focusElements[i].color.toLowerCase()){
                    this.setState({ value: values[j] });
                    return;
                }
            }
        }
    ...
    render() {
        ...
        const { value, display, pageX, pageY,values } = this.state;
        return (
            <div>
                <button onClick={e => this.displayChange(e)} style={{ ...styles, background: value, borderWidth: 0 }}></button>
                <PickerColor
                    values={values}
